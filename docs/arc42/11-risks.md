...
| Auswirkung | hoch — Verletzung von Quality-Szenario [Q02](./10-quality.md#q02--solver-performance) (≤ 2 s) |
...
| Test-Anker | UC11 → AC11.2 (siehe [use-cases.md UC11](../use-cases.md#uc11--auto-solve)) |
...
| Auswirkung | kritisch — Verletzung von [V07](../validation.md#v07--puzzle-solvability-uc05) und Top-Ziel **Q2** |
...
| Attribut | Wert |
|----------|------|
| Beschreibung | Der INSTEAD-/AFTER-Trigger `trg_CageCell_UniquePerPuzzle` (siehe [db/sudoku.sql](../../db/sudoku.sql) ab Zeile 99) ist die einzige DB-seitige Garantie, dass eine Zelle nicht in zwei Cages desselben Puzzles liegt. Fehler im Trigger oder Batch-Inserts mit deaktiviertem Trigger könnten inkonsistente Daten zulassen. |
| Wahrscheinlichkeit | niedrig |
| Auswirkung | hoch — Datenintegritäts-Verletzung gegenüber [V06](../validation.md#v06--cage-struktur-uc04) |
...
| Attribut | Wert |
|----------|------|
| Beschreibung | Blazor-Server-Apps halten pro User eine SignalR-Circuit (siehe [functionality.md](../functionality.md) "SignalR-Verbindung permanent"). Bei Pause + Tab-Wechsel kann die Circuit getrennt werden — laufende Game-State-Manipulationen würden verloren gehen, falls UC13-Auto-Save nicht greift. |
| Wahrscheinlichkeit | mittel |
...
| Attribut | Wert |
|----------|------|
| Beschreibung | Solver hat viele Code-Pfade (Cage mit 1 Zelle, Cage über ganze Zeile, unlösbares Puzzle, mehrdeutiges Puzzle, leeres Grid). Ohne gezielten Edge-Case-Plan kann die 80%-Marke (Quality-Szenario [Q06](./10-quality.md#q06--test-coverage)) am Solver allein scheitern. |
| Wahrscheinlichkeit | mittel |
| Auswirkung | hoch — verletzt Top-Ziel **Q3** und damit Submissions-Qualität |
| Mitigation | Solver-Unit-Tests systematisch nach der Edge-Case-Liste in [functionality.md "Kritische Abhängigkeiten"](../functionality.md#kritische-abhängigkeiten-für-test-order) (5+ Tests: README-Beispiele, unlösbar, mehrdeutig, 1-Zell-Cage, voller Zeilen-Cage). |
...
| Mitigation | `IGameService.CheckSolutionAsync` ruft als ersten Schritt den Sum-Check (siehe [V09](../validation.md#v09--sum-check-lösung-uc09)) auf; Unit-Test verifiziert, dass bei `Summe ≠ 405` der Solver nicht aufgerufen wird. |
...
| Mitigation | Schema in [db/sudoku.sql](../../db/sudoku.sql) ohne Solution-Spalte; Integration-Test gegen `INFORMATION_SCHEMA.COLUMNS` stellt sicher, dass keine `Solution`-Spalte existiert (siehe [erm.md](../erm.md#ableitungen-für-tests)). |
...
| Mitigation | Umbenennung gemäss [erm.md Design-Entscheidungen](../erm.md#design-entscheidungen): Tabelle `AppUser` statt `User`, Spalten `RowIdx` / `ColIdx` statt `Row`/`Col`. Bereits konsistent in [db/sudoku.sql](../../db/sudoku.sql) umgesetzt. |
| Test-Anker | Build-/Migrations-Lauf des `sudoku.sql`-Scripts |
...
| Attribut | Wert |
|----------|------|
| Beschreibung | EF-Migrations sind Source of Truth (`KillerSudoku.Data/Persistence/Migrations/`). Das von README §1.3 geforderte [db/sudoku.sql](../../db/sudoku.sql) wird per `dotnet ef migrations script --idempotent -o db/sudoku.sql` aus den Migrations exportiert. |
| Begründung | README §1.3 verlangt explizit ein `sudoku.sql`-Skript für den Prüfer; gleichzeitig braucht ein .NET-/EF-Workflow Migrations für reproduzierbare Schema-Evolution. Beides parallel zu pflegen ist günstiger als nur eine Variante. |
| Folge bei Produktiv-Use | Bei manuellen Änderungen am SQL-Skript ohne entsprechende Migration entsteht Drift — Disziplin nötig: Schema-Änderungen immer über Migrations, dann `sudoku.sql` neu exportieren. Im CI kann ein Drift-Check ergänzt werden (`dotnet ef migrations script` ↔ commited `sudoku.sql` vergleichen). |
...
| Begründung | Wettkampf-Scope. UC13 AC13.3 fordert lediglich "Nur 1 aktives Game pro User-Puzzle-Kombi gleichzeitig" — das ist über den DB-Filtered-Index `UX_Game_ActiveOnly` (siehe [db/sudoku.sql](../../db/sudoku.sql) Zeile 155) abgesichert, nicht über App-Tests. |
...
- [§10.2 Quality-Szenarien](./10-quality.md#102-quality-szenarien) — die durch Risiken bedrohten Antwortmasse
- [Validation V01–V16](../validation.md)
- [Use Cases mit Acceptance Criteria](../use-cases.md)
- [ER-Modell und Design-Entscheidungen](../erm.md)
- [DB-Schema](../../db/sudoku.sql)
