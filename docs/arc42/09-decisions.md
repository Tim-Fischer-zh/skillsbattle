...
- Das Image wird bei jedem `main`-Push via [`.github/workflows/deploy.yml`](../../.github/workflows/deploy.yml) gebaut und nach `ghcr.io/tim-fischer-zh/killer-sudoku:latest` gepusht → Prüfer kann mit einem `docker pull` ohne Local-Build verifizieren.
...
→ Vollständiges Setup in [§7.6 Production-Deployment](./07-deployment.md#76-production-deployment-bonus-live-demo-unter-web17skillcom).
...
| [ADR-016](#adr-016--production-deployment-via-self-hosted-runner--cloudflare-tunnel) | Production-Deploy via Self-Hosted Runner + Cloudflare-Tunnel | [§7.6](./07-deployment.md#76-production-deployment-bonus-live-demo-unter-web17skillcom) |
