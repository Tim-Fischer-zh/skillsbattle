...
> Bezug: [Kapitel 5](05-building-blocks.md) · [`../use-cases.md`](../use-cases.md) · [`../functionality.md`](../functionality.md) · [`../validation.md`](../validation.md) · [`../erm.md`](../erm.md)

Dieses Kapitel beschreibt die wichtigsten **Laufzeit-Szenarien** als Sequenz-Diagramme. Service-Methoden-Namen entsprechen exakt dem Service-Interface-Block in [`../functionality.md`](../functionality.md) §Service-Interfaces (Anti-Halluzinations-Anker). DB-Tabellen sind in [`../../db/sudoku.sql`](../../db/sudoku.sql) belegt.
...
**Beteiligt:** UC04 + UC05 · [V05](../validation.md#v05--difficulty-uc04) · [V06](../validation.md#v06--cage-struktur-uc04) · [V07](../validation.md#v07--puzzle-solvability-uc05) · Screen S5.
...
**Beteiligt:** UC07 · [V11](../validation.md#v11--hint-nur-bei-unvollständigem-grid-uc07) · Screen S6 · `IHintService` + `ISolverService`.
...
**Beteiligt:** UC09 · [V09](../validation.md#v09--sum-check-lösung-uc09) · [V10](../validation.md#v10--lösungs-validierung-vollständig-uc09) · Screen S6 · `IGameService`.
...
**Ziel:** User pausiert ein laufendes Spiel; Server friert Timer ein und persistiert den Zustand; bei Resume wird der Timer um die Pausen-Dauer korrigiert weitergeführt. Formel aus [`../erm.md`](../erm.md): `TimeSeconds = DATEDIFF(SECOND, StartTime, EndTime) − TotalPausedSeconds`.

**Beteiligt:** UC13 · [V12](../validation.md#v12--game-state-konsistenz-uc10-uc13) · Screen S6 · `IGameService.PauseAsync`/`ResumeAsync`.
...
- **Timer-Berechnung** bleibt in der DB-Formel verankert (siehe Comment-Block in `sudoku.sql` Zeilen 125–127): bei Spiel-Abschluss (UC10) gilt `TimeSeconds = DATEDIFF(SECOND, StartTime, EndTime) − TotalPausedSeconds`.
- **Filtered Unique Index** `UX_Game_ActiveOnly` ([`../erm.md`](../erm.md)) garantiert AC13.3: ein User kann pro Puzzle nur **ein** nicht abgeschlossenes Game haben — verhindert Pause-im-Spiel-1 + Neu-Start-Spiel-2-vor-Resume-Doubles.
...
- **Authorization** ([V16](../validation.md#v16--authorization-alle-geschützten-seitenendpoints)): jede Mutation prüft `Game.UserId == currentUserId` und antwortet mit 403 bei Mismatch.

---

> **Nächstes Kapitel:** [Kapitel 7 — Verteilungssicht](07-deployment.md) (TBD) — Deployment-Diagramm: lokale Dev-Workstation, MS-SQL-Express-Instanz, Build-/Run-Konfiguration.
