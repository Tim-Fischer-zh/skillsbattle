...
Datenzugriff über **Entity Framework Core 10** mit eigenem `SudokuDbContext`. Migrations werden **nicht** mit EF-Migrations gemacht, sondern das DB-Schema wird über ein deklaratives SQL-Script ([`sudoku.sql`](../../db/sudoku.sql)) angelegt — das entspricht der README-Vorgabe (siehe README §1.3, Submission-Artefakt).
...
- README §1.3 fordert explizit ein `sudoku.sql`-Skript mit CREATE-Statements
- Idempotenz wird durch Drop-If-Exists am Skript-Anfang gewährleistet (siehe [`sudoku.sql`](../../db/sudoku.sql) Zeilen 22–31)
