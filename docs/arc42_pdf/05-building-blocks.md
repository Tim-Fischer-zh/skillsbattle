<h1 id="chapter-5">5 Bausteinsicht</h1>

> arc42 v8.2 · Kapitel 5 · Killer Sudoku
> Bezug: [Kapitel 4](#chapter-4) · **Funktionalitäts-Matrix** (autoritativ für Service-Signaturen) · **ER-Modell** · **Datenbank-Skript**

Dieses Kapitel zerlegt das System statisch in seine Bausteine. Die Service-Signaturen werden **nicht** dupliziert — sie stehen in **Funktionalitäts-Matrix** §Service-Interfaces. Hier wird die Architektur-Sicht beschrieben: Zweck, Schnittstellen, enthaltene Klassen, Abhängigkeiten.
...
Für jeden Baustein: Zweck · Schnittstellen (eingehend/ausgehend) · enthaltene Klassen/Komponenten. Service-Signaturen siehe **Funktionalitäts-Matrix**.
...
| Aspekt | Details |
|--------|---------|
| **Zweck** | EF-Core-Persistenz; Mapping C#-Entities ↔ T-SQL-Tabellen aus **Datenbank-Skript**; Query-Optimierung; View-Zugriff. |
...
Vollständiges Schema in **Datenbank-Skript**. Folgende Objekte werden angelegt:
...
- **Trigger `trg_CageCell_UniquePerPuzzle`** — verhindert, dass eine Zelle in mehreren Cages desselben Puzzles liegt. Wirft `THROW 50001` bei Verletzung (siehe **Datenbank-Skript** Zeilen 100–121).
...
> **Nächstes Kapitel:** [Kapitel 6 — Laufzeitsicht](#chapter-6) — Sequenz-Diagramme der vier wichtigsten Szenarien (Puzzle-Save, Hint, Check-Solution, Pause/Resume).
