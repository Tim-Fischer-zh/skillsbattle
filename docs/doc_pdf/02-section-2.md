...
**ORM:** Entity Framework Core 10 (`SudokuDbContext`) — Schema-Verwaltung via **Datenbank-Skript**; EF-Modellierung spiegelt das SQL-Schema (kein `dotnet ef migrations` aktiv).
...
> **Anmerkung Kardinalitäten:** `Puzzle ||--|{ Cage` (1..*): ein valide gespeichertes Puzzle hat mindestens einen Cage (insgesamt 81 Zellen müssen abgedeckt sein). DB-seitig garantiert dies aktuell nicht ein expliziter Constraint, sondern die Application-Validation in `PuzzleStructureValidator` (siehe **Validation-Regeln** V06).
...
| Highscore | DB-Read über LINQ-Joins (`Game ⋈ AppUser ⋈ Puzzle`) im `HighscoreService`; **View `vw_Highscore` existiert im Schema** (**Datenbank-Skript**) als Reserve, wird vom aktuellen Service jedoch **nicht** gemappt | Service-Code referenziert `_db.Games` direkt; View-Switch bleibt offen, ohne dass das Schema angepasst werden muss |
...
|------------|-------|--------------|
| `AppUser.UserName` UNIQUE (filtered, WHERE NOT NULL) | DB | Identity-Standard; Pflicht-Length 3–50 und Pattern werden in der App (**V01**) durchgesetzt — kein CHECK-Constraint im SQL |
| `AppUser.Email` UNIQUE (filtered, WHERE NOT NULL) | DB | Identity-Standard; Format-Validation läuft in der App (**V02**) — kein CHECK-Constraint im SQL |
